...
public class CampManagementApplication {
    ...
    private static void inquireRoundGradeBySubject() {
        ...
        // 기능 구현
        System.out.println("조회할 수강생의 이름을 입력해주세요.");
        System.out.print("이름 입력: ");
        String name = sc.next();

        int studentId;
        ArrayList<Student> students = studentRepository.findByName(sc.next());
        if (students.isEmpty()) {
            return; // 이름 발견 안 됐을 때 입력 다시 받도록 구현 필요.
        } else if (students.size() == 1) {
            studentId = students.get(0).getId();
        } else {
            System.out.println("중복된 이름이 발견되었습니다. ID를 입력해주세요.");
            for (Student student : students) {
                System.out.println(student.getId() + ". " + student.getName());
            }
            studentId = sc.nextInt(); // students에 없는 id 입력 가능하기 때문에 studentId 유효성 검증 추가 구현 필요.
            sc.nextLine();
        }

        Student student = studentRepository.findById(studentId);
        Map<Subject, Score> report = student.getReport().getReport();
        ...
        int subjectId = sc.nextInt();
        Score score = report.get(Subject.findById(subjectId));
        score.printScore();
        System.out.println("\n등급 조회 성공!");
    }
